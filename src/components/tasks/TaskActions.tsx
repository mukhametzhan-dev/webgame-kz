...
export function TaskActions({
  ...
  return (
    <div className="mt-6">
      {checked && (
        <div
          className={`animate-pop-in mb-4 rounded-2xl px-4 py-3 text-center font-bold ${
            percent >= 100
      ...
      <div className="flex gap-3">
        {!checked ? (
          <button
            onClick={onCheck}
            disabled={!canCheck}
            className="flex-1 rounded-2xl bg-teal-500 py-3 font-extrabold text-white shadow-md transition enabled:hover:bg-teal-600 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {checkLabel}
          </button>
        ) : (
          <>
            {percent < 100 && (
              <button
                onClick={onRetry}
                className="flex-1 rounded-2xl bg-white py-3 font-extrabold text-slate-500 shadow-md ring-1 ring-slate-200 transition hover:bg-slate-50"
              >
                Қайталау
              </button>
            )}
